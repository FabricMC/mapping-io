...
package net.fabricmc.mappingio.visitor;
...
public class VisitEndTest {
	...
	public static void setup() throws Exception {
		dirs.add(TestHelper.DETECTION_DIR);
		dirs.add(TestHelper.VALID_DIR);
		dirs.add(TestHelper.VALID_WITH_HOLES_DIR);
	}
	...
	public void testVisitEnd() throws Exception {
		for (MappingFormat format : MappingFormat.values()) {
			...
			for (Path dir : dirs) {
				MappingReader.read(dir.resolve(filename), format, new VisitEndTestVisitor(1));

				VisitEndTestVisitor threePassVisitor = new VisitEndTestVisitor(3);
	...
	private static class VisitEndTestVisitor implements MappingVisitor {
		private VisitEndTestVisitor(int visitPassCountToFinish) {
			this.visitPassCountToFinish = visitPassCountToFinish;
		}

		@Override
		public Set<MappingFlag> getFlags() {
			return visitPassCountToFinish > 0
